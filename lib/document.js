...
const { isReferenceable } = require('./validate');
const { isEmbeddedDocument } = require('./validate');
      ...
      .then(
        data =>
          options !== undefined && options.select
            ? _.pick(data, ['_schema', '_id', ...options.select])
            : data
      )
      .then(docs => {
        if (docs) {
    ...
    return DB()
      ...
      .then(
        data =>
          options !== undefined && options.select
            ? _.map(data, datum =>
                _.pick(datum, ['_schema', '_id', ...options.select])
              )
            : data
      )
      .then(docs => {
